...
## BUG-06 — Tela branca total ao tocar em "Mapa" após Multi-Vertical V2

**Data da correção:** 2026-06-21
**Commit de correção:** (ver abaixo) — "fix: remover useTerminologia de MapaCalor para resolver circular ESM (BUG-06)"
**Identificado por:** Eduardo ao validar V2+V3 em produção
**Tempo em produção:** ~horas (commits V2/V3 foram 7251e3d e 179d57c, bug corrigido no mesmo dia)

### Descrição

Após o deploy do Multi-Vertical V2 (commit 7251e3d), abrir a tela Mapa de Calor causava tela branca total — o header do app desaparecia e nem o spinner do Suspense era exibido.

### Causa raiz

O V2 adicionou `import { useTerminologia } from "../contexts/TerminologiaContext"` em `MapaCalor.tsx` para adaptar os textos "contato/contatos" no Popup e no rodapé.

`TerminologiaContext.tsx` reside no bundle principal (`index-[hash].js`). Com o import, o Rolldown inseriu uma linha de import estático no topo do chunk lazy `MapaCalor-[hash].js`:

```js
import{a as e,c as t,...,h as r,...}from"./index-C4fwGADK.js";
// r = alias de h = função t de useTerminologia
```

Isso criou uma **dependência ESM circular**:

```
index-C4fwGADK.js
  └── dynamic import (React.lazy) ──→ MapaCalor-[hash].js
                                            └── static import ──→ index-C4fwGADK.js
                                                                        ↑ CIRCULAR
```

Quando o browser avalia o chunk MapaCalor (triggado pelo dynamic import), encontra o static import de volta ao bundle principal. O live binding `r` (função `t`) pode estar em estado não-inicializado nesse ponto, resultando em `TypeError: r is not a function`. Como não há `ErrorBoundary` na árvore, a raiz desmonta → tela branca sem spinner.

**Por que o build passou:** circular ESM é sintaticamente válida. TypeScript e Rolldown validam estrutura, não ordem de inicialização em runtime. O crash só se manifesta no browser.

**Por que o spinner do Suspense não apareceu:** Suspense captura Promises suspensas; quando o módulo falha (Promise rejeitada), o erro propaga para cima sem mostrar o fallback.

### O que foi corrigido

**`src/pages/MapaCalor.tsx`:**
- Removido `import { useTerminologia } from "../contexts/TerminologiaContext"`
- Removido `const { t } = useTerminologia()` do componente
- Revertidos os dois textos dinâmicos para pluralização hardcoded em português:
  - Popup: `{g.total} contato{g.total !== 1 ? "s" : ""}`
  - Rodapé: `{semLocalizacao} contato{semLocalizacao !== 1 ? "s" : ""} sem localização geocodificada...`

O chunk lazy voltou a ser isolado. O BUG-05 (lazy-load do MapaCalor) permanece 100% intacto.

### Trade-off aceito

`MapaCalor.tsx` exibe "contato/contatos" em português genérico, independente do vertical. As outras 9 telas do V2 continuam com terminologia dinâmica completa.

### Lição

**Componentes carregados via `React.lazy()` NÃO podem importar de módulos que residem no bundle principal se esses módulos são exportados por um Context Provider também no bundle principal.** O import estático cross-chunk cria uma dependência circular que o browser pode não resolver corretamente em runtime.

Se for necessário passar dado dinâmico para um componente lazy, use props ou uma store externa isolada (Zustand, Jotai) que não crie referência circular no grafo de módulos.
