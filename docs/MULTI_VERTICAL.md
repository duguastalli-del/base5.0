...
## Os 8 verticais suportados

| Vertical | contato | contatos | novo_contato | base_contatos | captador | captadores | operacao |
|---|---|---|---|---|---|---|---|
| `politica` | Apoiador | Apoiadores | Novo Apoiador | Base de Apoiadores | Voluntário | Voluntários | Campanha |
| `religioso` | Membro | Membros | Novo Membro | Comunidade | Liderança | Lideranças | Ação Pastoral |
| `imobiliario` | Lead | Leads | Novo Lead | Pipeline | Corretor | Corretores | Captação |
| `varejo` | Cliente | Clientes | Novo Cliente | Base de Clientes | Vendedor | Vendedores | Promoção |
| `pesquisa` | Pesquisado | Pesquisados | Novo Pesquisado | Amostra | Pesquisador | Pesquisadores | Pesquisa |
| `publicidade` | Lead | Leads | Novo Lead | Base | Agente | Agentes | Ativação |
| `ong` | Apoiador | Apoiadores | Novo Apoiador | Rede de Apoio | Voluntário | Voluntários | Ação |
| `outro` | Contato | Contatos | Novo Contato | Base de Contatos | Agente | Agentes | Operação |
...
| `captadores` | Plural de captador (ex: "Voluntários") |
...
**Nota:** a UI de edição de vocabulário ainda não existe. As customizações precisam ser feitas diretamente no banco (SQL ou Supabase Dashboard → Table Editor → workspace_settings).

---

## Telas adaptadas (V2 — completo)

Todas as telas do sistema usam `useTerminologia()`:
...
Antoniassi 2026 (vertical='politica') já tem settings (inseridos pela migration 000009) → fluxo 100% inalterado.

---

## Limitações conhecidas

### MapaCalor.tsx não usa useTerminologia (BUG-06 + BUG-07)

`src/pages/MapaCalor.tsx` está desativado (BUG-07). O componente existe em disco como código morto.

**Regra permanente:** componentes em `React.lazy()` não podem importar do TerminologiaContext. Ver `docs/BUGS_RESOLVIDOS.md#BUG-06`.
...
### V2 (2026-06-21) ✅
- Todas as 9 telas adaptadas (ver tabela acima)
